...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  Widget _buildMessage(Map<String, dynamic> msg, int messageIndex) {
    ...
    Color? backgroundColor = isUser ? Colors.blue[100] : Colors.grey[300];
    if (isCurrentHighlight) {
      backgroundColor = Colors.yellow[200]; // Highlight current match
    } else if (isSearchMatch) {
      backgroundColor = Colors.blue.withOpacity(0.3); // Highlight other matches
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.search, color: Colors.white), // Toggle in-chat search
